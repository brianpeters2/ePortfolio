...
namespace
{
const char* const WINDOW_TITLE = "Tutorial 6.1"; // Macro for window title
...
glm::vec3 gCubePosition(1.0f, 0.0f, 0.0f);
...
glm::vec4 gLightColor(1.0f, 1.0f, 1.0f, 1.0f); //added and made it as vec4 Brian Peters 
...
const GLchar * cubeFragmentShaderSource = GLSL(440,
    ...
    void main()
    {
       ...
       // std::vector<Vertex> mesh = loadOBJ("../../resources/textures/Lamp.obj");
...
const GLchar * lampVertexShaderSource = GLSL(440,
    ...
    layout (location = 0) in vec3 position; // VAP position 0 for vertex position data
flat out int iflip; // Into vertex shader OUT to fragment shader Added BP 
        //Uniform / Global variables for the  transform matrices
    uniform mat4 model;
    uniform mat4 view;
    uniform mat4 projection;
    uniform int flip; // where to find function - into vertex shader Added BP 


    void main()
    {
        //std::vector<Vertex> mesh = loadOBJ("../../resources/textures/Lamp.obj"); //obj file to lamp(light)
        gl_Position = projection * view * model * vec4(position, 1.0f); // Transforms vertices into clip coordinates
        iflip = flip; // connecting functions Added BP 
...
flat in int iflip; // taking codes from iflip and flip 
    void main()
    {
        if (iflip == 1) {
        ...
        while (flipActive) {
            flip = (flip % 5) + 1; // Cycle through 1 to 5
            cout << "flip = " << flip << endl;

            // Simulate updating fragmentColor based on flip
            if (iflip == 1) {
                cout << "fragmentColor = vec4(1.0f, 1.0f, 1.0f, 1.0f)" << endl;
            }
            else if (iflip == 2) {
                cout << "fragmentColor = vec4(1.0f, 0.0f, 0.0f, 1.0f)" << endl;
            }
            else if (iflip == 3) {
                cout << "fragmentColor = vec4(1.0f, 1.0f, 0.0f, 1.0f)" << endl;
            }
            else if (iflip == 4) {
                cout << "fragmentColor = vec4(1.0f, 0.0f, 1.0f, 1.0f)" << endl;
            }
            else if (iflip == 5) {
                cout << "fragmentColor = vec4(0.0f, 1.0f, 1.0f, 1.0f)" << endl;
            }
            else {
                cout << "fragmentColor = vec4(0.0f, 0.0f, 1.0f, 1.0f)" << endl;
            }
...
int flip = 1; //Added flipping color BP
...
int main(int argc, char* argv[])
{
    ...
    // Load texture
   // loadOBJ("../../resources/textures/Lamp.obj");

    /*
    const char* texFilename = "../../resources/textures/Lamp.obj";
    ...
    */
    ...
    while (!glfwWindowShouldClose(gWindow))
    {
        ...
        //Right click to automatically cycle ONCE BP
        elapse += gDeltaTime;
        if (elapse > 1.0 && flipActive) {
            flip = (flip + 1) % 5;
            numColors++;
            elapse = 0;
            cout << flip << endl;
...
void UProcessInput(GLFWwindow* window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_I) == GLFW_PRESS)
        gLightPosition.z -= 0.1f;
    if (glfwGetKey(window, GLFW_KEY_K) == GLFW_PRESS)
        gLightPosition.z += 0.1f;
    if (glfwGetKey(window, GLFW_KEY_J) == GLFW_PRESS)
        gLightPosition.x -= 0.1f;
    if (glfwGetKey(window, GLFW_KEY_L) == GLFW_PRESS)
        gLightPosition.x += 0.1f;
    if (glfwGetKey(window, GLFW_KEY_U) == GLFW_PRESS)
        gLightPosition.y -= 0.1f;
    ...
    switch (button)
    {
        case GLFW_MOUSE_BUTTON_LEFT:
        {
            if (action == GLFW_PRESS) {
               ...
               // flip = flip == 1 ? 0 : 1; // flipping color three options added BP 
                flip = (flip + 1) % 5; // Cycle through 0 to 4
            }
            else
        ...
        case GLFW_MOUSE_BUTTON_RIGHT:
        {
           ...
           flipActive = true;
           // thread(main).detach(); // Start the flip update in a separate thread
        }
            else {
...
void URender()
{
   ...
   // glUseProgram(gLampProgramId); //Added for texture to lamp BP
    ...
    */
    ...
    glUniform3f(lightColorLoc, gLightColor.r, gLightColor.g, gLightColor.b);      //lightcolor mental note BP
    ...
    //----------------
    glUseProgram(gLampProgramId); 
    GLuint gflip = glGetUniformLocation(gLampProgramId, "flip"); //locate the flip variable 
    glUniform1i(gflip, flip); //setting flip into vertex shader 
...
void UCreateMesh(GLMesh &mesh)
{
    ...
    GLfloat verts[] = {
        ...
        // --------------------------------------
        // 
       -5.0f, -5.0f,-5.0f,  1.0f, 1.0f,  0.0f,    //Added plane Brian Peters
